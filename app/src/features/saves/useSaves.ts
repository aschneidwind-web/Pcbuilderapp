...
export const useSaves = (): UseSavesResult => {
  ...
  // snapshot captures builds before the optimistic update so it's available
  // synchronously in the catch block — the functional-updater side-effect approach
  // doesn't work because React may defer running the updater past the await.
